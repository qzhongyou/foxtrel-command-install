...
function _beforeInstall() {
    return new Promise((resolve, reject)=> {
        // is it gennerater ?
        if (!this.is) {
            log.error(`${this.setting.filePath} not is generater`);
        } else {
            if (this.generaterRealPath) {
                ...
                packageJson(this.setting.filePath).then((data)=> {
                    ...
                    if (data.version == curVersion) {
                        log.debug("generater already had been installed && generater is up-to-date");
                        resolve(false);
                    } else {
                        log.debug("generater update version: " + data.version);
                        resolve(true);
                    }
                }).catch(err=> {
                    log.error(err);
            ...
            if (yield _beforeInstall.bind(me)()) {
                ...
                child.on("close", function (code, signal) {
                    if (code == 0) {
                        ...
                        log.error('Failed to installed a generator.\n'.red);
                    }
                });
                /**
