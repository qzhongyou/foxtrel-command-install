...
var Generater = function (setting) {
    ...
    co(function *() {
        //执行安装
        if (yield Generater._install.bind(me)()) {
            //初始化,执行模块
            gulp.task("init", Generater._init.bind(me));

            // 提问
            gulp.task("props", ['init'], Generater.base.prompting.bind(me));

            //构建
            gulp.task("create", ['init', "props"], Generater._create.bind(me));
        }
    })
};
